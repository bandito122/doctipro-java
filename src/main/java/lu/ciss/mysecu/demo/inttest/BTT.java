...
/**
 * D�monstrations des appels au service de test d'int�gration BTT de mySecu.
 * Il s'agit de tester l'int�gration d'un jeton d'authentification dans un message SOAP
 * avec contenu arbitraire.
 *
 * URL du service: /ws/soap/inttest/btt
 *
 * ATTENTION:
 ...
 * <li> args[12]: chemin vers le message SOAP � envoyer </li>
 * <li> args[13]: l'URL compl�te du service BTT </li>
 * </ol>
 *
 * @author u156gm
 * @version $Id: BTT.java 16672 2015-06-30 14:26:21Z u156gm $
 * @since REL_1-0-1
 */
public class BTT {
     ...
     * {@inheritDoc}
     */
    public static void main(String[] args) throws IOException, SOAPException, MarshallingException,
        ...
        // 11.) chargement du message SOAP � envoyer
        Document xmlDocument = XmlTools.loadFromFile(args[12]);
        xmlDocument.normalize();

        // 12.) signature du message SOAP
        xmlDocument = XmlDSig_OpenSAML.signBody(xmlDocument, args[1], args[2], args[3],
                args[7]);

        // 13.) ajout du jeton d'authentification dans l'ent�te WS-Security
        Element enteteWSSE = (Element) xmlDocument.getElementsByTagNameNS(Security.ELEMENT_NAME.getNamespaceURI(), Security.ELEMENT_NAME.getLocalPart()).item(0);
        enteteWSSE.appendChild(xmlDocument.importNode(authToken, true));

        // 14.) r�cup�ration de la connexion vers le serveur
        url = new URL(args[13]);
        connection = HTTPClient.getHTTPConnection(url, args[1], args[2],
                args[3], args[4], args[5], args[6]);

        // 15.) envoie du message au serveur
